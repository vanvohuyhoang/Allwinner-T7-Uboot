...
int sunxi_clock_get_corepll(void)
{
	...
	div_k = ((reg_val >> 4) & 0x3) + 1;
	div_m    = ((reg_val >> 0) & 0x3) + 1;

	clock = (24 * factor_n * div_k)/div_m/div_p;
...
static void set_corepll(u32 reg_addr, int frequency)
{
	unsigned int reg_val = 0;
...
int sunxi_clock_set_corepll(int frequency, int core_vol)
{
	...
	/*set corepll*/
	set_corepll(CCMU_PLL_C0_CPUX_CTRL_REG, frequency);
	set_corepll(CCMU_PLL_C1_CPUX_CTRL_REG, frequency);
